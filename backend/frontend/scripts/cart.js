...
function renderProduct() {
    ...
    console.log(product_data);
        ...
        <tr> 
            ...
            <td id="quantity"><input type="number" value="${product.quantity}"></td>
...
function Order() {
    document.getElementById('purchase-btn-wrapper').addEventListener("click", async function() {
       const cart =  localStorage.getItem('cart');
       const res = await fetch(
        `http://localhost:3000/api/v1/customer/order`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
